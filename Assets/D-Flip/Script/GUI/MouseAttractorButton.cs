...
public class MouseAttractorButton : MonoBehaviour {
    ...
    public void Select(string key)
    {
        if (key == "カラー")
        {
            SystemManager.Instance.RemoveAttractor(SystemManager.AttractorState.KEYWORD);
            SystemManager.Instance.AddAttractor(SystemManager.AttractorState.COLOR);
            gameObject.transform.Find("Text").GetComponent<Text>().text = "Color";
        }
        else
        {
            SystemManager.Instance.RemoveAttractor(SystemManager.AttractorState.COLOR);
            PhotoManager.Instance.mouseAttractorKey = PhotoManager.Instance.keywordCode[key];
            SystemManager.Instance.AddAttractor(SystemManager.AttractorState.KEYWORD);
